...
class CoTLayer(nn.Module):
    ...
    def forward(self, query, context, key_padding_mask=None):
        ...
        attn_out, _ = self.mha(query=q, key=context, value=context, attn_mask=attn_mask, key_padding_mask=key_padding_mask)
...
class DistilCoTAdapter(nn.Module):
    def __init__(self, backbone_name="distilgpt2", nrepeat=3, freeze_backbone=True):
        ...
        self.cot_layer = CoTLayer(self.embed_dim, num_heads=num_heads, ff_dim=self.embed_dim * 4, dropout=0.1)
    ...
    def forward(self, input_ids, attention_mask=None):
        ...
        for r in range(self.nrepeat):
            ...
            key_pad_bool = (key_pad == 0)  # True where padded -> MHA expects True for masked positions
            query = prev[-1]
            refined = self.cot_layer(query=query, context=ctx, key_padding_mask=key_pad_bool)
